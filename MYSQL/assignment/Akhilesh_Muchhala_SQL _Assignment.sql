...
#2
SELECT account_id FROM account WHERE open_date BETWEEN date '2002-01-01' and '2002-12-31' ; 
...
SELECT account_id,product_cd FROM account WHERE account_id IN(1,10,23,27);
 ...
 #5
SELECT account_id FROM account WHERE avail_balance BETWEEN 100 and 200 ;
...
*/
 ...
 #3
 SELECT * FROM account ORDER BY account_id  LIMIT 2,2 ;
 
 #4
SELECT YEAR(birth_date),MONTH(birth_date),DAY(birth_date),WEEKDAY(birth_date) FROM individual;

#5
SELECT SUBSTRING('Please find the substring in this string',17,9) AS str;

#6
SELECT ABS(-25.76823) AS Abs , SIGN(-25.76823) As Sign , ROUND(-25.76823,-2) AS rounded ;

#7
SELECT date_add( '2022/06/12', INTERVAL 30 DAY) AS DateAdd ;

#8
 SELECT LEFT(fname,3) as first_name,RIGHT(lname,3) as last_name FROM individual ;
 ...
 */
#1
SELECT cust_id , COUNT(account_id) AS Number_of_accounts FROM account  GROUP BY  cust_id ;
 
 #2
 SELECT cust_id , COUNT(account_id) AS Number_of_accounts FROM account GROUP BY cust_id HAVING COUNT(account_id)>2;

#3
SELECT fname,birth_date FROM individual ORDER BY birth_date DESC ;

#4
SELECT YEAR(open_date) AS YEAR ,AVG(avail_balance) AS AVERAGE
        FROM account 
          GROUP BY YEAR(open_date) HAVING AVG(avail_balance)>200 
             ORDER BY open_date ;

#5 
SELECT product_cd , MAX(pending_balance) FROM account GROUP BY product_cd HAVING product_cd IN ('CHK','SAV','CD'); 
...
*/
#1
SELECT fname AS first_name , title , name FROM employee e JOIN department d ON  e.dept_id = d.dept_id;
...
#3
 ...
 select s.fname superior_first, s.lname superior_last, 
              e.fname employee_first, e.lname employee_last 
...
#5
 ...
 select emp_id, fname, lname from employee where emp_id  IN 
